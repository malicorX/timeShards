...
/** Parse datetime-local value to RFC3339 UTC-ish string for API. */
export function fromLocalDatetimeInputValue(value: string): string {
  return new Date(value).toISOString();
}

/** ISO timestamp → value for datetime-local input (local timezone). */
...
/** Short German display for validity / audit timestamps. */
export function formatIsoLocalShort(iso: string | null | undefined): string {
  if (!iso) return '';
  const d = new Date(iso);
...
function berlinMidnightIso(ymd: Ymd): string {
...
/** Monday 00:00 Berlin – next Monday 00:00 Berlin (matches API `week_bounds_utc`). */
...
/** Default shift: tomorrow 08:00–16:00 Berlin. */
export function defaultShiftRange(): { start: string; end: string } {
  const tomorrow = addYmdDays(berlinYmd(new Date()), 1);
  const startYmd = { ...tomorrow };
  const endYmd = tomorrow;
  const startBase = berlinMidnightIso(startYmd);
  const start = new Date(new Date(startBase).getTime() + 8 * 3_600_000).toISOString();
  const end = new Date(new Date(startBase).getTime() + 16 * 3_600_000).toISOString();
  return { start, end };
}
